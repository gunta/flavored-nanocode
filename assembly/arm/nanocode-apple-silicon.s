...
.p2align 7
body:       .skip 16384         // JSON body (escape expansion)
.p2align 7
req:        .skip 32768         // Full HTTP request
.p2align 7
...
// Input: x0 = buffer start (JSON body)
...
//=============================================================================
// MAIN API CALL FUNCTION (Keep-alive + DNS cache + JSON escape)
//=============================================================================
call_api:
    stp     x29, x30, [sp, #-160]!
    mov     x29, sp
    ...
    adrp    x28, flags@PAGE
    add     x28, x28, flags@PAGEOFF
    ldr     x27, [x28]
    mov     x25, #0             // retry flag

.L2_try:
    //--- Load API key once ---
    tbz     x27, #FLAG_KEY, .L2_load_key
.L2_key_ok:
    //--- Build JSON body ---
    adrp    x0, body@PAGE
...
.L2_body_done:
    ...
    adrp    x1, req_conn@PAGE
    add     x1, x1, req_conn@PAGEOFF
    bl      strcpy_simd
...
.L2_connect:
    ...
    mov     x0, x22
    ldr     x1, [x24, #32]      // ai_addr
    ldr     w2, [x24, #16]      // ai_addrlen
    mov     x16, #SYS_CONNECT
    svc     #0x80
    tbnz    x0, #63, .L2_error
...
.L2_conn_ok:
    ...
    mov     x19, #0             // total read
    mov     x21, #0             // header end offset
    mov     x22, #0             // content length
    movz    x26, #0xFFFF
    movk    x26, #0x1, lsl #16   // RESP_MAX
...
.L2_read_loop:
    ...
    // Find header end once
    cbnz    x21, .L2_check_done
    mov     x0, x9
    mov     x1, x19
    bl      find_header_end
    cbz     x0, .L2_read_loop
